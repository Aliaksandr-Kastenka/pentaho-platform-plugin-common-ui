    ...
    function (Base, Base64Util, Formatter, Parameter, ParameterDefinition, ParameterGroup, ParameterValue, $, Logger) {
      ...
      var _determineParamsValidation = function (paramDefn) {
        for(var j=0; j<paramDefn.getParameterGroup("parameters").parameters.length; j++) {
          var dependents = _parseDependents(paramDefn, paramDefn.getParameterGroup("parameters").parameters[j].name); // downstream dependency
          if(dependents.length > 0) {
            paramDefn.getParameterGroup("parameters").parameters[j].canBeValidatedOnClientSide = false; // if this parameter will be changed - server validation
